...
const tests: TestCase[] = [
  ...
  {
    name: "blocked profile denies valid zion admin access",
    run: () => {
      const result = resolveAccessState(
        createInput({
  ...
  {
    name: "blocked profile wins over store readiness",
    run: () => {
      const result = resolveAccessState(
        createInput({
          hasProfile: true,
  ...
  {
    name: "inactive memberships deny with 403",
    run: () => {
      const result = resolveAccessState(
        createInput({
          hasAnyMemberships: true,
