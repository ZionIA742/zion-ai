...
async function handleDeleteItem(itemId: string) {
  ...
  const filteredItems = useMemo(() => {
    ...
    const next = items.filter((item) => {
      ...
      const stockStatus = String(item.stock_status || "").trim().toLowerCase();
    ...
    if (sortBy === "price_asc" || sortBy === "price_desc") {
      return [...next].sort((a, b) => {
        const aPrice =
          typeof a.price_cents === "number" ? a.price_cents : Number.POSITIVE_INFINITY;
        const bPrice =
          typeof b.price_cents === "number" ? b.price_cents : Number.POSITIVE_INFINITY;
        return sortBy === "price_asc" ? aPrice - bPrice : bPrice - aPrice;
      });
    }

    return next;
  }, [items, searchTerm, statusFilter, stockFilter, sortBy]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, stockFilter, sortBy]);
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <details ref={advancedActionsRef} className="relative">
            ...
            <div className="absolute right-0 z-20 mt-2 w-72 rounded-xl border border-gray-200 bg-white p-2 shadow-xl">
              <button
                type="button"
              ...
              <p className="px-3 pb-1 pt-2 text-xs leading-5 text-gray-500">
                Use apenas quando quiser remover toda esta categoria da loja.
              </p>
            </div>
          </details>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-3">
        <div className="flex flex-col gap-2 xl:flex-row xl:items-center">
          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5">
            ...
            <input
              ...
              placeholder={`Buscar em ${pageTitle.toLowerCase()} por nome, SKU, marca ou modelo...`}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2 px-1 text-xs text-gray-500">
          ...
          {(searchTerm.trim() || statusFilter !== "all" || stockFilter !== "all") ? (
            <button
              type="button"
              onClick={() => {
                setSearchTerm("");
      ...
      {!hasValidStoreContext ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
          ...
          {visibleItems.map((item) => {
            ...
            const primaryItemPhotoUrl = primaryItemPhoto
              ? photoUrlByPhotoId[primaryItemPhoto.id] || ""
              : "";
            ...
            return (
              <section key={item.id} className={isExpanded || isEditing ? "bg-gray-50/40" : "bg-white"}>
                <div className="grid gap-3 px-4 py-3.5 transition hover:bg-gray-50 lg:grid-cols-[minmax(280px,1.5fr)_minmax(170px,0.75fr)_minmax(120px,0.55fr)_minmax(210px,0.9fr)_96px] lg:items-center lg:gap-4">
                  ...
                  >
                    <div className="flex h-14 w-16 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-gray-100 text-[11px] font-semibold text-gray-400">
                      {primaryItemPhotoUrl ? (
                        <img
                          src={primaryItemPhotoUrl}
                          alt={primaryItemPhoto?.file_name || item.name}
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 truncate text-sm text-gray-500">
                        {cleanLooseText(item.metadata?.brand) || "Sem marca"}
                      </div>
                    </div>
                  </button>
                  ...
                  <div className="pl-[76px] text-sm text-gray-700 lg:pl-0">
                    ...
                    <div className="mt-1 truncate text-xs text-gray-500">
                      {cleanLooseText(item.metadata?.line || item.metadata?.model) || "Sem linha/modelo"}
                    </div>
                  </div>
                ...
                {isExpanded || isEditing ? (
                  ...
                  {isEditing && editForm ? (
                    <div className="rounded-2xl bg-gray-50 p-4">
                      <div className="mb-4">
                        <div className="text-sm font-bold text-gray-950">Editar item</div>
          ...
          {filteredItems.length > pageSize ? (
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 bg-gray-50 px-4 py-3">
              <div className="text-xs text-gray-500">
                Mostrando {(safeCurrentPage - 1) * pageSize + 1}–{Math.min(safeCurrentPage * pageSize, filteredItems.length)} de {filteredItems.length}
              </div>
