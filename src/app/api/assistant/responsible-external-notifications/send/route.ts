...
import { sendResponsibleExternalNotification } from "@/lib/server/assistant/responsible-external-notifications-sender";
import {
...
type SendNotification = typeof sendResponsibleExternalNotification;
...
export function createResponsibleExternalNotificationsSendPostHandler(
  deps: PostHandlerDeps = {},
) {
  const resolveAccess = deps.resolveAccess ?? resolveStoreApiAccess;
  const sendNotification =
    deps.sendNotification ?? sendResponsibleExternalNotification;

  return async function handleResponsibleExternalNotificationsSendPost(
    request: Request,
  ) {
    try {
      ...
      return buildJsonResponse(
        {
          ok: false,
          sent: false,
          reason: "RESPONSIBLE_EXTERNAL_NOTIFICATION_SEND_FAILED",
          message:
            error?.message ||
            "Nao foi possivel enviar a notificacao externa do responsavel.",
        },
        500,
      );
    }
...
export const POST = createResponsibleExternalNotificationsSendPostHandler();
