...
export default function Sidebar() {
  ...
  const [waitingHumanAttentionCount, setWaitingHumanAttentionCount] = useState(0);
  ...
  const loadInboxCounter = useCallback(async () => {
    ...
    const count = rows.filter(isWaitingHumanAttention).length;
    setWaitingHumanAttentionCount(count);
      ...
      <nav className="flex-1 space-y-2 p-4">
        {items.map((item) => {
          ...
          const showInboxBadge =
            isInboxItem && waitingHumanAttentionCount > 0;
