...
-- Forward migration for the approved Commercial Strategy redesign.
-- Keeps the previous structured fields for history, but introduces explicit
-- canonical authorities for deal priorities, avoid-cases and AI action.

alter table public.store_strategy_settings
  add column if not exists strategy_priority_deal_types text[] null,
  add column if not exists strategy_priority_deal_types_other text null,
  add column if not exists strategy_avoid_cases text[] null,
  add column if not exists strategy_avoid_cases_other text null,
  add column if not exists strategy_avoid_action text null,
  add column if not exists strategy_commercial_strategy_configured_at timestamptz null;

comment on column public.store_strategy_settings.strategy_priority_deal_types is
  'Canonical business/deal types the store wants Sales AI to prioritize. This is a preference and does not prohibit other valid sales.';

comment on column public.store_strategy_settings.strategy_priority_deal_types_other is
  'Free-text complement when another priority business/deal type is selected.';

comment on column public.store_strategy_settings.strategy_avoid_cases is
  'Canonical observable commercial situations the store prefers not to pursue aggressively.';

comment on column public.store_strategy_settings.strategy_avoid_cases_other is
  'Free-text complement for another observable commercial situation the store prefers to avoid.';

comment on column public.store_strategy_settings.strategy_avoid_action is
  'Canonical default Sales AI action when an explicitly configured avoid-case is observed.';

comment on column public.store_strategy_settings.strategy_commercial_strategy_configured_at is
  'Timestamp of the first explicit valid save of the redesigned Commercial Strategy card.';
...
begin
  if not exists (
    ...
    where conrelid = 'public.store_strategy_settings'::regclass
      and conname = 'store_strategy_settings_redesign_shape_valid'
  ) then
    alter table public.store_strategy_settings
      add constraint store_strategy_settings_redesign_shape_valid
      check (
        strategy_commercial_strategy_configured_at is not null
        or (
          strategy_priority_deal_types is null
          and strategy_priority_deal_types_other is null
          and strategy_avoid_cases is null
          and strategy_avoid_cases_other is null
          and strategy_avoid_action is null
        )
      );
  end if;
end;
...
begin
  ...
  foreach v_candidate in array coalesce(p_strategy_sell_more_choices, '{}'::text[]) loop
    ...
    if v_candidate <> '' then
      if not (v_candidate = any(array['piscinas', 'piscinas_instalacao', 'acessorios', 'quimicos', 'servicos', 'troca_equipamentos', 'sem_prioridade', 'outro']::text[])) then
