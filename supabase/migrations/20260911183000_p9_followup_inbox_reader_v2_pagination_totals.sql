...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:b5:e5.5.3:followup-inbox-reader-v2-pagination-totals:v1',
    0
  )
);
...
begin
  if pg_catalog.to_regprocedure(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'precondition failed: followup inbox reader v1 is missing';
  end if;
  ...
  if pg_catalog.to_regprocedure(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'precondition failed: followup inbox reader v2 already exists';
  end if;
  ...
  if pg_catalog.to_regclass('public.conversation_sessions') is null
     ...
     or pg_catalog.to_regclass('public.commercial_opportunity_followups') is null then
    raise exception using
      errcode = 'P0001',
...
begin
  if v_user_id is null or v_request_role <> 'authenticated' then
    raise exception using
      errcode = '42501',
      message = 'followup opportunity candidates v2 are not authorized';
  end if;

  if p_organization_id is null then
    raise exception using
      errcode = '22023',
      message = 'followup candidates v2 require organization';
  end if;
  ...
  if p_store_id is not null
    ...
    raise exception using
      errcode = '23503',
      message = 'followup candidate v2 store scope not found';
  end if;

  v_followup_type := pg_catalog.lower(pg_catalog.btrim(coalesce(p_followup_type, 'offer')));

  if v_followup_type not in ('offer', 'visit') then
    raise exception using
      errcode = '22023',
      message = 'followup candidate v2 type is not supported';
  end if;

  v_suggested_action :=
    case
      when v_followup_type = 'visit' then 'followup_visit'
      else 'followup_offer'
    end;
  ...
  base as (
    select
      ...
      v_followup_type as followup_type,
      v_suggested_action as suggested_action,
  ...
  order by
    ...
    counted.followup_type asc
  limit v_limit
  offset v_offset;
end;
$function$;

alter function public.panel_list_followup_opportunity_candidates_scoped_v2(
  uuid,
  uuid,
  text,
  integer,
...
revoke all on function public.panel_list_followup_opportunity_candidates_scoped_v2(
  uuid,
  uuid,
  text,
  integer,
...
grant execute on function public.panel_list_followup_opportunity_candidates_scoped_v2(
  uuid,
  uuid,
  text,
  integer,
...
comment on function public.panel_list_followup_opportunity_candidates_scoped_v2(
  uuid,
  uuid,
  text,
  integer,
  ...
  'P9 5.5.3 Inbox follow-up reader v2. Adds deterministic offset pagination, canonical totals and persisted follow-up fields without changing v1 callers.';
...
begin
  if v_oid is null then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: followup inbox reader v2 is missing';
  end if;

  select pg_catalog.lower(pg_catalog.pg_get_functiondef(v_oid))
  into v_definition;

  if pg_catalog.strpos(v_definition, 'total_count') = 0
     or pg_catalog.strpos(v_definition, 'offset v_offset') = 0
     or pg_catalog.strpos(v_definition, 'commercial_opportunity_followups') = 0
     or pg_catalog.strpos(v_definition, 'operational_state') = 0
     or pg_catalog.strpos(v_definition, 'count(*) filter') = 0
     or pg_catalog.strpos(v_definition, 'followup_type asc') = 0 then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: followup inbox reader v2 contract is incomplete';
  end if;
  ...
  if exists (
    ...
    where namespace.nspname = 'public'
      and proc.proname = 'panel_list_followup_opportunity_candidates_scoped'
      and pg_catalog.pg_get_function_identity_arguments(proc.oid)
        = 'p_organization_id uuid, p_store_id uuid, p_followup_type text, p_min_hours_since_customer integer, p_limit integer, p_offset integer'
  ) then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: v1 reader signature was changed instead of creating v2';
  end if;
end;
$postconditions$;
