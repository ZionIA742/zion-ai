...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('zion:p9:checklist-human-override-writer:v1', 0)
);

-- ============================================================================
-- P9 / Bloco 3 / Etapa 3.5
-- Canonical Human Checklist Applicability Override Writer v1.
--
-- Contract:
-- - authenticated human only; actor identity comes exclusively from auth.uid();
-- - active membership is mandatory;
-- - exact organization/store/opportunity scope;
-- - caller must present the checklist version it actually reviewed;
-- - replay is resolved before stale-current validation;
-- - one override changes exactly one applicability item;
-- - human may choose required|optional|not_applicable|needs_resolution;
-- - human may resolve a system conflict but may never create conflict manually;
-- - every override creates an immutable direct-child checklist version and an
--   append-only override event;
-- - all non-target items are cloned unchanged;
-- - profile/policy/settings snapshot authority is inherited from the base;
-- - checklist_state is derived again from the resulting item states;
-- - same operation_key + identical payload is idempotent; reuse with a
--   divergent payload fails closed;
-- - direct table writes remain closed to runtime roles.
-- ============================================================================

do $preflight$
declare
  v_table text;
  v_signature text := 'public.override_commercial_opportunity_checklist_item_by_user(uuid,uuid,uuid,uuid,text,text,text,text,text,text,text,jsonb)';
begin
  foreach v_table in array array[
    ...
    if pg_catalog.to_regclass('public.' || v_table) is null then
      raise exception using
        errcode = 'P0001',
        message = pg_catalog.format('precondition failed: public.%s is required', v_table);
    end if;
  end loop;
  ...
  if pg_catalog.to_regprocedure(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'precondition failed: checklist override validation trigger function is required';
  end if;

  if pg_catalog.to_regprocedure(v_signature) is not null then
    raise exception using
      errcode = 'P0001',
      message = 'precondition failed: canonical checklist human override writer already exists';
  end if;
end;
$preflight$;

create function public.override_commercial_opportunity_checklist_item_by_user(
...
-- ============================================================================
-- Postconditions / hardening contract.
-- ============================================================================
do $postconditions$
declare
...
begin
  if v_writer is null then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: checklist human override writer is missing';
  end if;
  ...
  if pg_catalog.has_function_privilege('anon', v_writer, 'EXECUTE')
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: checklist human override writer grants mismatch';
  end if;
  ...
  if pg_catalog.strpos(v_definition, 'auth.uid()') = 0
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: checklist human override writer authority contract mismatch';
  end if;
  ...
  if v_owner is distinct from 'postgres'
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: checklist human override writer hardening mismatch';
  end if;
  ...
  if pg_catalog.has_table_privilege('authenticated', 'public.commercial_opportunity_checklist_versions', 'INSERT')
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: direct checklist write privileges leaked';
  end if;
end;
$postconditions$;
