...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:b5:e5.1:followup-inbox-canonical-bridge-reader-fix:v1',
    0
  )
);
...
begin
  if v_function_oid is null then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: canonical followup inbox reader is missing';
  end if;
  ...
  if pg_catalog.strpos(
       v_normalized,
       'select card_source.* from public.panel_list_crm_opportunity_cards_scoped'
     ) = 0
     or pg_catalog.strpos(
       v_normalized,
       'card_source.conversation_id is not null'
     ) = 0 then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: qualified conversation reader fix is missing';
  end if;
  ...
  if pg_catalog.strpos(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: ambiguous conversation_id reference remains';
  end if;
end;
$postconditions$;
