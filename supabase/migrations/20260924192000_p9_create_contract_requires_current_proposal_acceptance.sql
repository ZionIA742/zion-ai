...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:create-contract-current-proposal-acceptance:v2',
    0
  )
);
...
--
-- create_contract now requires human-confirmed acceptance of the exact
-- lifecycle + quote + quote_version exposed by Current Commercial Proposal.
--
-- Historical/stale acceptance never authorizes a new contract.
-- Current Proposal conflict/needs_resolution remains fail-closed with its
-- semantic class preserved.
-- ============================================================================
...
begin
  if pg_catalog.to_regprocedure(
    ...
    raise exception using
      errcode = 'P0001',
      message =
        'precondition failed: commercial action readiness resolver is required';
  end if;
  ...
  if pg_catalog.to_regprocedure(
    ...
    raise exception using
      errcode = 'P0001',
      message =
        'precondition failed: private Current Proposal Acceptance resolver is required';
  end if;
end;
$preconditions$;
...
begin
  ...
  begin
    ...
    if v_action_key = 'create_contract' then
      ...
      -- accepted exact Current Commercial Proposal and a required technical visit, when applicable.
      select item_row.*
      into v_quote_item
      from public.commercial_opportunity_checklist_items item_row
      where item_row.organization_id = p_organization_id
      ...
      -- ----------------------------------------------------------------------
      -- P9 6.8: contract creation requires explicit human-confirmed acceptance
      -- of the exact Current Commercial Proposal. The private acceptance
      -- authority preserves Current Proposal none/conflict/needs_resolution
      -- semantics and resolves stale historical acceptance deterministically.
      -- ----------------------------------------------------------------------

      select acceptance_row.*
      into v_proposal_acceptance
      ...
      if not found then
        v_state := 'conflict';
        v_reason := 'create_contract_current_proposal_acceptance_missing';
        exit resolution;
      end if;

      v_details := v_details || pg_catalog.jsonb_build_object(
        'proposal_acceptance_state',
          v_proposal_acceptance.acceptance_state,
        'proposal_acceptance_reason_code',
          v_proposal_acceptance.reason_code,
        'proposal_acceptance_event_id',
          v_proposal_acceptance.acceptance_event_id,
        'proposal_acceptance_source_message_id',
          v_proposal_acceptance.source_message_id,
        'proposal_acceptance_accepted_at',
          v_proposal_acceptance.accepted_at,
        'proposal_acceptance_stale_count',
          v_proposal_acceptance.stale_acceptance_count,
        'current_proposal_quote_id',
          v_proposal_acceptance.current_quote_id,
        'current_proposal_quote_version_id',
          v_proposal_acceptance.current_quote_version_id
      );

      if v_proposal_acceptance.acceptance_state = 'accepted' then
        if v_proposal_acceptance.reason_code
             is distinct from 'current_proposal_accepted'
           or v_proposal_acceptance.acceptance_event_id is null
           or v_proposal_acceptance.current_quote_id is null
           or v_proposal_acceptance.current_quote_version_id is null then
          v_state := 'conflict';
          v_reason :=
            'create_contract_current_proposal_acceptance_invalid';
          exit resolution;
        end if;

      elsif v_proposal_acceptance.acceptance_state = 'none'
            and v_proposal_acceptance.reason_code =
              'current_proposal_not_accepted' then
        v_state := 'blocked';
        v_reason := 'create_contract_current_proposal_not_accepted';
        v_blockers := pg_catalog.jsonb_build_array(
          pg_catalog.jsonb_build_object(
            'item_key', 'proposal_acceptance',
            'acceptance_state',
              v_proposal_acceptance.acceptance_state,
            'reason_code',
              v_proposal_acceptance.reason_code,
        ...
        exit resolution;
            ...
            and v_proposal_acceptance.reason_code =
              'current_proposal_acceptance_stale' then
        v_state := 'blocked';
        v_reason := 'create_contract_current_proposal_acceptance_stale';
        v_blockers := pg_catalog.jsonb_build_array(
          pg_catalog.jsonb_build_object(
            'item_key', 'proposal_acceptance',
            'acceptance_state',
      ...
      elsif v_proposal_acceptance.acceptance_state =
            'needs_resolution' then
        v_state := 'needs_resolution';
        v_reason :=
          'create_contract_current_proposal_acceptance_needs_resolution';
        exit resolution;

      elsif v_proposal_acceptance.acceptance_state = 'conflict' then
        v_state := 'conflict';
        v_reason :=
          'create_contract_current_proposal_acceptance_conflict';
        exit resolution;

      elsif v_proposal_acceptance.acceptance_state = 'none' then
        -- "none" inherited from an unresolved Current Proposal is not a known
        -- customer refusal/non-acceptance. It remains unresolved authority.
        v_state := 'needs_resolution';
        v_reason :=
          'create_contract_current_proposal_acceptance_needs_resolution';
        exit resolution;
  ...
  v_basis := pg_catalog.jsonb_build_object(
    'schema', 'p9_commercial_action_readiness_v2',
...
comment on function public.p9_resolve_commercial_action_readiness_internal(
  ...
  'P9 Commercial Action Readiness v2. create_contract requires acceptance of the exact canonical Current Commercial Proposal through the private P9 6.8 acceptance authority; stale acceptance blocks, unresolved/conflicting Current Proposal fails closed, and other action semantics remain unchanged.';
...
begin
  if v_function is null then
    raise exception using
      errcode = 'P0001',
      message =
        'postcondition failed: action readiness resolver missing';
  end if;
  ...
  if v_definition not like
     ...
     or v_definition not like
       $p9_pattern$%'resolver_version', 2%$p9_pattern$
     or v_definition like
       '%from public.commercial_proposal_acceptance_events%'
     or v_definition ~*
       'order[[:space:]]+by[[:space:]]+(created_at|updated_at|version_number)[[:space:]]+desc'
     or v_definition ~*
       'max[[:space:]]*\([[:space:]]*version_number'
     or v_definition ~*
       'limit[[:space:]]+1' then
    raise exception using
      errcode = 'P0001',
      message =
        'postcondition failed: create_contract acceptance readiness contract mismatch';
  end if;
  ...
  if pg_catalog.has_function_privilege(
    ...
    raise exception using
      errcode = 'P0001',
      message =
        'postcondition failed: internal action readiness resolver became exposed';
  end if;
end;
$postconditions$;
