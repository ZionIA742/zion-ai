...
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('zion:p9:priority:message-asof-hotfix:v1',0));

-- P9 / Bloco 5 / Etapa 5.4
-- Corrective migration after 20260910150000 and 20260911130000 were applied remotely.
-- Preserves the reason-code fix and bounds message evidence by evaluated as_of.
...
begin
  ...
  where message_row.organization_id = p_organization_id
    ...
    and message_row.created_at <= v_as_of;
...
comment on function public.p9_resolve_commercial_opportunity_priority_internal(uuid,uuid,uuid,timestamp with time zone) is
  'P9 5.4 canonical commercial opportunity priority resolver. Reason codes use explicit array_append semantics; message evidence is bounded by evaluated as_of.';
