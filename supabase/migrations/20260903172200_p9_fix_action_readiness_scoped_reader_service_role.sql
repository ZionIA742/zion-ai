...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('zion:p9:fix-action-readiness-scoped-reader-service-role:v1', 0)
);

-- ============================================================================
-- P9 3.5-H1 hotfix - Action Readiness scoped reader service-role safety.
--
-- The original scoped reader initialized auth.uid() in DECLARE for every
-- caller. Supabase auth.uid() casts request.jwt.claim.sub to uuid, so a
-- service_role request with an empty sub failed before the role branch could
-- authorize service_role. Keep the same authorization/readiness contract but
-- resolve auth.uid() only inside the authenticated branch.
-- ============================================================================
...
begin
  if pg_catalog.to_regprocedure(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'precondition failed: scoped commercial action readiness reader is required';
  end if;
...
create or replace function public.read_commercial_action_readiness_scoped(
...
comment on function public.read_commercial_action_readiness_scoped(
  ...
  'Scoped runtime bridge for P9 commercial action readiness. Authenticated callers require active organization membership and exact store/opportunity scope; service_role/postgres are server/admin contexts. auth.uid() is evaluated only for authenticated requests. Commercial semantics remain owned by the internal action-specific resolver.';
...
begin
  if v_function is null then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: scoped action readiness reader missing';
  end if;
  ...
  if pg_catalog.has_function_privilege(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: internal action readiness resolver became runtime-executable';
  end if;

  if pg_catalog.strpos(v_definition, 'zion_resolve_request_role_internal') = 0
     or pg_catalog.strpos(v_definition, 'authenticated') = 0
     or pg_catalog.strpos(v_definition, 'service_role') = 0
     or pg_catalog.strpos(v_definition, 'postgres') = 0
     or pg_catalog.strpos(v_definition, 'v_user_id := auth.uid()') = 0
     or pg_catalog.strpos(v_definition, 'p9_resolve_commercial_action_readiness_internal') = 0 then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: scoped action readiness reader delegation/role shape mismatch';
  end if;

  if pg_catalog.strpos(v_definition, 'v_user_id uuid := auth.uid()') > 0 then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: auth.uid() is still evaluated unconditionally';
  end if;
end;
$postconditions$;
