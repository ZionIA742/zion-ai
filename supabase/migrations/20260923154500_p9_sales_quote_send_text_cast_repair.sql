...
-- Repair: restore truncated PostgreSQL text casts in functions installed by
-- 20260923132000_p9_sales_quote_send_pre_provider_linearization.sql.
--
-- The already-applied migration is intentionally not rewritten or replayed.
-- ============================================================================
...
begin
  ...
  select message_row.id
   ...
   where message_row.organization_id = new.organization_id
     ...
     and coalesce(message_row.metadata ->> 'sales_quote_id', '') = new.quote_id::text
     and coalesce(message_row.metadata ->> 'sales_quote_version_id', '') =
           v_quote.current_version_id::text
     and (
...
begin
  ...
  into v_gate_function;

  if v_trigger_function not ilike '%new.quote_id::text%'
     or v_trigger_function not ilike '%v_quote.current_version_id::text%' then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: trigger function text casts not repaired';
  end if;

  if v_gate_function not ilike '%v_quote_id::text%'
     or v_gate_function not ilike '%v_version_id::text%'
     or v_gate_function not ilike '%v_opportunity_id::text%'
     or v_gate_function not ilike '%v_block_reason)::text%' then
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: gate v2 text casts not repaired';
  end if;
  ...
  if not exists (
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: linearization trigger missing';
  end if;
  ...
  if has_function_privilege(
    ...
    raise exception using
      errcode = 'P0001',
      message = 'postcondition failed: gate v2 ACL changed unexpectedly';
  end if;
end;
$postconditions$;
