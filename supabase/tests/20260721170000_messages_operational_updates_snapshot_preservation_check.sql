-- ZION / Pilar 9 / Bloco 4 / Etapa 4.20
-- Um unico teste: atualizacoes operacionais preservam o snapshot comercial.
--
-- Esperado:
-- - a mensagem nasce com commercial_context_capture_state = 'captured';
-- - mark_message_external_sent() registra o external_message_id;
-- - delivered_at e read_at podem ser atualizados;
-- - conversation_session_id permanece inalterado;
-- - commercial_session_context_link_id permanece inalterado;
-- - commercial_context_capture_state permanece inalterado.
--
-- O runner:
-- - reutiliza uma conversa real apenas como parent imutavel;
-- - cria oportunidade, sessao e contexto proprios;
-- - cria a mensagem somente via public.insert_message();
-- - executa mark_message_external_sent() como service_role;
-- - atualiza delivered_at e read_at como service_role;
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.20:captured_context_message',
    0
  )
);

create temp table _p9_420_result (
...
declare
  ...
  v_original_capture_state text;
  v_original_session_id uuid;
  v_original_context_link_id uuid;

  v_external_message_id text;
  v_delivered_at timestamp with time zone;
  v_read_at timestamp with time zone;

  v_mark_external_ok boolean := false;
  v_mark_external_sqlstate text;
  v_mark_external_error text;

  v_delivery_read_ok boolean := false;
  v_delivery_read_sqlstate text;
  v_delivery_read_error text;
  v_delivery_read_row_count bigint := 0;

  v_operational_values_ok boolean := false;
  v_snapshot_unchanged boolean := false;
  v_operational_updates_ok boolean := false;
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_420_result values (
      1,
      'CHECK',
      'BLOCKED',
  ...
  begin
    ...
    select (
      public.link_commercial_session_context(
        ...
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_20',
  ...
  if not v_context_created or not v_context_ready then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.20 nao conseguiu criar contexto ativo (rpc_ok=%s, context_id=%s, ready=%s, sqlstate=%s, error=%s)',
        v_context_rpc_ok,
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.20 ' || v_run_id::text,
        null,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_20',
  ...
  into
    v_original_capture_state,
    v_original_session_id,
    v_original_context_link_id
  from public.messages message_row
  where message_row.id = v_message_id;

  v_external_message_id :=
    'runner-ext-'
    || pg_catalog.replace(v_run_id::text, '-', '')
    || '-420';

  v_delivered_at := pg_catalog.clock_timestamp();
  v_read_at := v_delivered_at + interval '1 second';
  ...
  begin
    ...
    perform public.mark_message_external_sent(
      v_message_id,
      v_external_message_id
    );

    v_mark_external_ok := true;
  exception
    when others then
      get stacked diagnostics
        v_mark_external_sqlstate = returned_sqlstate,
        v_mark_external_error = message_text;
      v_mark_external_ok := false;
  end;
  ...
  begin
    ...
    set delivered_at = v_delivered_at,
        read_at = v_read_at
    where id = v_message_id;

    get diagnostics
      v_delivery_read_row_count = row_count;

    v_delivery_read_ok := v_delivery_read_row_count = 1;
  exception
    when others then
      get stacked diagnostics
        v_delivery_read_sqlstate = returned_sqlstate,
        v_delivery_read_error = message_text;
      v_delivery_read_ok := false;
  end;
  ...
  select exists (
    ...
    from public.messages message_row
    where message_row.id = v_message_id
      and message_row.external_message_id = v_external_message_id
      and message_row.delivered_at is not distinct from v_delivered_at
      and message_row.read_at is not distinct from v_read_at
  )
  into v_operational_values_ok;

  select exists (
    select 1
    from public.messages message_row
    where message_row.id = v_message_id
      and message_row.commercial_context_capture_state
          is not distinct from v_original_capture_state
      and message_row.conversation_session_id
          is not distinct from v_original_session_id
      and message_row.commercial_session_context_link_id
          is not distinct from v_original_context_link_id
  )
  into v_snapshot_unchanged;

  v_operational_updates_ok :=
    v_mark_external_ok
    and v_delivery_read_ok
    and v_delivery_read_row_count = 1
    and v_operational_values_ok
    and v_snapshot_unchanged;
  ...
  insert into pg_temp._p9_420_result values (
    ...
    case
      when v_opportunity_created
       ...
       and v_snapshot_ok
       and v_operational_updates_ok
      then 'PASS'
      else 'SUT_FAIL'
    end,
    pg_catalog.format(
      'conversation_id=%s | opportunity_id=%s | session_id=%s | context_link_id=%s | message_id=%s | initial_snapshot_ok=%s | external_message_id=%s | mark_external_ok=%s | delivery_read_ok=%s | delivery_read_row_count=%s | operational_values_ok=%s | snapshot_unchanged=%s | operational_updates_ok=%s | mark_sqlstate=%s | mark_error=%s | delivery_read_sqlstate=%s | delivery_read_error=%s | context_rpc_ok=%s | insert_ok=%s | context_sqlstate=%s | context_error=%s | insert_sqlstate=%s | insert_error=%s',
      v_conversation_id,
      v_opportunity_id,
      v_session_id,
      coalesce(v_context_link_id::text, '<null>'),
      ...
      v_snapshot_ok,
      coalesce(v_external_message_id, '<null>'),
      v_mark_external_ok,
      v_delivery_read_ok,
      v_delivery_read_row_count,
      v_operational_values_ok,
      v_snapshot_unchanged,
      v_operational_updates_ok,
      coalesce(v_mark_external_sqlstate, '<null>'),
      coalesce(v_mark_external_error, '<null>'),
      coalesce(v_delivery_read_sqlstate, '<null>'),
      coalesce(v_delivery_read_error, '<null>'),
      v_context_rpc_ok,
      v_insert_ok,
      coalesce(v_context_sqlstate, '<null>'),
      coalesce(v_context_error, '<null>'),
  ...
  if not v_cleanup_ok then
    raise exception using
      errcode = 'P0001',
      message = pg_catalog.format(
        'Etapa 4.20 cleanup failed; transaction rolled back (messages=%s, contexts=%s, sessions=%s, opportunities=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, context_trigger=%s, identity_clean=%s)',
...
exception
  when others then
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message =
            'Etapa 4.20 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.20 safety gate falhou; transacao revertida';
    end if;
