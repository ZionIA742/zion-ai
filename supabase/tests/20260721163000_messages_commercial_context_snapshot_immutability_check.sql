-- ZION / Pilar 9 / Bloco 4 / Etapa 4.19
-- Um unico teste: imutabilidade do snapshot comercial da mensagem.
--
-- Esperado:
-- - a mensagem nasce com commercial_context_capture_state = 'captured';
-- - conversation_session_id guarda a sessao ativa;
-- - commercial_session_context_link_id guarda o contexto ativo;
-- - tentativa de alterar cada uma das tres colunas falha com P0001;
-- - cada falha usa a mensagem oficial de imutabilidade;
-- - a linha permanece exatamente com o snapshot original.
--
-- O runner:
...
-- - executa tres UPDATEs negativos, um por coluna protegida;
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.19:captured_context_message',
    0
  )
);

create temp table _p9_419_result (
...
declare
  ...
  v_capture_update_sqlstate text;
  v_capture_update_error text;
  v_session_update_sqlstate text;
  v_session_update_error text;
  v_context_update_sqlstate text;
  v_context_update_error text;

  v_snapshot_unchanged boolean := false;
  v_immutability_ok boolean := false;
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_419_result values (
      1,
      'CHECK',
      'BLOCKED',
  ...
  begin
    ...
    select (
      public.link_commercial_session_context(
        ...
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_19',
  ...
  if not v_context_created or not v_context_ready then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.19 nao conseguiu criar contexto ativo (rpc_ok=%s, context_id=%s, ready=%s, sqlstate=%s, error=%s)',
        v_context_rpc_ok,
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.19 ' || v_run_id::text,
        null,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_19',
  ...
  begin
    ...
    set commercial_context_capture_state = 'pending_context'
    where id = v_message_id;
  exception
    when others then
      get stacked diagnostics
        v_capture_update_sqlstate = returned_sqlstate,
        v_capture_update_error = message_text;
  end;
  ...
  exception
    when others then
      get stacked diagnostics
        v_session_update_sqlstate = returned_sqlstate,
        v_session_update_error = message_text;
  end;

  begin
    update public.messages
    set commercial_session_context_link_id = null
    where id = v_message_id;
  exception
    when others then
      get stacked diagnostics
        v_context_update_sqlstate = returned_sqlstate,
        v_context_update_error = message_text;
  end;
  ...
  v_immutability_ok :=
    v_capture_update_sqlstate = 'P0001'
    and v_session_update_sqlstate = 'P0001'
    and v_context_update_sqlstate = 'P0001'
    and v_capture_update_error =
      'messages commercial context snapshot is immutable after insert'
    and v_session_update_error =
      'messages commercial context snapshot is immutable after insert'
    and v_context_update_error =
      'messages commercial context snapshot is immutable after insert'
  ...
  insert into pg_temp._p9_419_result values (
    ...
    pg_catalog.format(
      'conversation_id=%s | opportunity_id=%s | session_id=%s | context_link_id=%s | message_id=%s | initial_snapshot_ok=%s | capture_update_sqlstate=%s | capture_update_error=%s | session_update_sqlstate=%s | session_update_error=%s | context_update_sqlstate=%s | context_update_error=%s | snapshot_unchanged=%s | immutability_ok=%s | context_rpc_ok=%s | insert_ok=%s | context_sqlstate=%s | context_error=%s | insert_sqlstate=%s | insert_error=%s',
      v_conversation_id,
      v_opportunity_id,
      v_session_id,
      coalesce(v_context_link_id::text, '<null>'),
      ...
      v_snapshot_ok,
      coalesce(v_capture_update_sqlstate, '<null>'),
      coalesce(v_capture_update_error, '<null>'),
      coalesce(v_session_update_sqlstate, '<null>'),
      coalesce(v_session_update_error, '<null>'),
      coalesce(v_context_update_sqlstate, '<null>'),
      coalesce(v_context_update_error, '<null>'),
  ...
  if not v_cleanup_ok then
    raise exception using
      errcode = 'P0001',
      message = pg_catalog.format(
        'Etapa 4.19 cleanup failed; transaction rolled back (messages=%s, contexts=%s, sessions=%s, opportunities=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, context_trigger=%s, identity_clean=%s)',
...
exception
  when others then
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message =
            'Etapa 4.19 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.19 safety gate falhou; transacao revertida';
    end if;
