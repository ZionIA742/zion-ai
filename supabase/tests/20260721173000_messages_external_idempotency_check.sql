-- ZION / Pilar 9 / Bloco 4 / Etapa 4.21
-- Um unico teste: idempotencia por external_message_id.
--
-- Esperado:
-- - o primeiro insert_message() com external_message_id exclusivo funciona;
-- - a primeira mensagem nasce com snapshot captured correto;
-- - o segundo insert_message() usa o mesmo external_message_id;
-- - o segundo insert usa conteudo e metadata diferentes;
-- - a segunda tentativa falha com SQLSTATE 23505;
-- - apenas a primeira mensagem permanece durante a verificacao;
-- - nenhuma linha da segunda tentativa e criada.
--
-- O runner:
-- - reutiliza uma conversa real apenas como parent imutavel;
-- - cria oportunidade, sessao e contexto proprios;
-- - cria mensagens somente via public.insert_message();
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.21:captured_context_message',
    0
  )
);

create temp table _p9_421_result (
...
declare
  ...
  v_first_insert_sqlstate text;
  v_first_insert_error text;

  v_second_insert_ok boolean := false;
  v_second_insert_sqlstate text;
  v_second_insert_error text;
  v_second_constraint_name text;

  v_external_message_count bigint := 0;
  v_second_fixture_count bigint := 0;
  v_first_snapshot_ok boolean := false;
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_421_result values (
      1,
      'CHECK',
      'BLOCKED',
  ...
  begin
    ...
    select (
      public.link_commercial_session_context(
        ...
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_21',
  ...
  if not v_context_created or not v_context_ready then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.21 nao conseguiu criar contexto ativo (rpc_ok=%s, context_id=%s, ready=%s, sqlstate=%s, error=%s)',
        v_context_rpc_ok,
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.21 first ' || v_run_id::text,
        v_external_message_id,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_21',
  ...
  exception
    when others then
      get stacked diagnostics
        v_first_insert_sqlstate = returned_sqlstate,
        v_first_insert_error = message_text;
      v_first_insert_ok := false;
  end;
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.21 second DIFFERENT ' || v_run_id::text,
        v_external_message_id,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_21',
    ...
    v_second_insert_ok := true;
  exception
    when others then
      get stacked diagnostics
        v_second_insert_sqlstate = returned_sqlstate,
        v_second_insert_error = message_text,
        v_second_constraint_name = constraint_name;
      v_second_insert_ok := false;
  end;
  ...
  select count(*)
  into v_external_message_count
  from public.messages message_row
  where message_row.external_message_id = v_external_message_id;

  select count(*)
  into v_second_fixture_count
  from public.messages message_row
  ...
  into v_first_snapshot_ok;

  v_idempotency_ok :=
    v_first_insert_ok
    and v_message_id is not null
    and v_first_snapshot_ok
    and not v_second_insert_ok
    and v_second_message_id is null
    and v_second_insert_sqlstate = '23505'
    and v_external_message_count = 1
    and v_second_fixture_count = 0;
  ...
  insert into pg_temp._p9_421_result values (
    ...
    case
      when v_opportunity_created
       ...
       and v_context_ready
       and v_idempotency_ok
      then 'PASS'
      else 'SUT_FAIL'
    end,
    pg_catalog.format(
      'conversation_id=%s | opportunity_id=%s | session_id=%s | context_link_id=%s | first_message_id=%s | second_message_id=%s | external_message_id=%s | first_insert_ok=%s | first_snapshot_ok=%s | second_insert_ok=%s | second_sqlstate=%s | second_constraint=%s | external_message_count=%s | second_fixture_count=%s | idempotency_ok=%s | first_sqlstate=%s | first_error=%s | second_error=%s | context_rpc_ok=%s | context_ready=%s | context_sqlstate=%s | context_error=%s',
      v_conversation_id,
      v_opportunity_id,
      v_session_id,
      coalesce(v_context_link_id::text, '<null>'),
      coalesce(v_message_id::text, '<null>'),
      coalesce(v_second_message_id::text, '<null>'),
      coalesce(v_external_message_id, '<null>'),
      v_first_insert_ok,
      v_first_snapshot_ok,
      v_second_insert_ok,
      coalesce(v_second_insert_sqlstate, '<null>'),
      coalesce(v_second_constraint_name, '<null>'),
      v_external_message_count,
      v_second_fixture_count,
      v_idempotency_ok,
      coalesce(v_first_insert_sqlstate, '<null>'),
      coalesce(v_first_insert_error, '<null>'),
      coalesce(v_second_insert_error, '<null>'),
      v_context_rpc_ok,
      v_context_ready,
      coalesce(v_context_sqlstate, '<null>'),
      coalesce(v_context_error, '<null>')
  ...
  if not v_cleanup_ok then
    raise exception using
      errcode = 'P0001',
      message = pg_catalog.format(
        'Etapa 4.21 cleanup failed; transaction rolled back (messages=%s, contexts=%s, sessions=%s, opportunities=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, context_trigger=%s, identity_clean=%s)',
...
exception
  when others then
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message =
            'Etapa 4.21 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.21 safety gate falhou; transacao revertida';
    end if;
