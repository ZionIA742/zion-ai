-- ZION / Pilar 9 / Bloco 4 / Etapa 4.15
-- Um unico teste: mensagem nova com contexto comercial capturado.
--
-- Esperado:
-- commercial_context_capture_state = 'captured'
-- conversation_session_id = id da sessao ativa
-- commercial_session_context_link_id = id do contexto comercial ativo
--
-- O runner:
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.15:captured_context_message',
    0
  )
);

create temp table _p9_415_result (
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_415_result values (
      1,
      'CHECK',
      'BLOCKED',
  ...
  begin
    ...
    select (
      public.link_commercial_session_context(
        ...
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_15',
  ...
  if not v_context_created or not v_context_ready then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.15 nao conseguiu criar contexto ativo (rpc_ok=%s, context_id=%s, ready=%s, sqlstate=%s, error=%s)',
        v_context_rpc_ok,
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.15 ' || v_run_id::text,
        null,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_15',
  ...
  insert into pg_temp._p9_415_result values (
    ...
    pg_catalog.format(
      'conversation_id=%s | opportunity_id=%s | session_id=%s | context_link_id=%s | message_id=%s | context_rpc_ok=%s | context_ready=%s | insert_ok=%s | snapshot_ok=%s | context_sqlstate=%s | context_error=%s | insert_sqlstate=%s | insert_error=%s',
      v_conversation_id,
      v_opportunity_id,
      v_session_id,
      coalesce(v_context_link_id::text, '<null>'),
  ...
  if not v_cleanup_ok then
    raise exception using
      errcode = 'P0001',
      message = pg_catalog.format(
        'Etapa 4.15 cleanup failed; transaction rolled back (messages=%s, contexts=%s, sessions=%s, opportunities=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, context_trigger=%s, identity_clean=%s)',
...
exception
  when others then
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message =
            'Etapa 4.15 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.15 safety gate falhou; transacao revertida';
    end if;
