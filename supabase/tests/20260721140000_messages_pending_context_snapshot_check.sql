-- ZION / Pilar 9 / Bloco 4 / Etapa 4.14
-- Um unico teste: mensagem nova com sessao ativa e contexto pendente.
--
-- Esperado:
-- commercial_context_capture_state = 'pending_context'
-- conversation_session_id = id da sessao ativa
-- commercial_session_context_link_id = null
--
-- O runner reutiliza uma conversa real apenas como parent imutavel, cria uma
-- sessao ativa propria, cria uma mensagem outgoing via public.insert_message(),
-- remove as fixtures e confirma que a conversa permaneceu intacta.
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.14:pending_context_message',
    0
  )
);

create temp table _p9_414_result (
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_414_result values (
      1,
      'CHECK',
      'BLOCKED',
      'preflight bloqueado: usuario, objetos, permissao ou triggers obrigatorios ausentes',
      null
    );
    insert into pg_temp._p9_414_result values (
      2,
  ...
  if v_conversation_id is null then
    insert into pg_temp._p9_414_result values (
      ...
      'CHECK',
      'BLOCKED',
      'nenhuma conversa elegivel sem sessao ativa foi encontrada para criar a fixture',
      null
    );
    insert into pg_temp._p9_414_result values (
      2,
  ...
  select exists (
    ...
    where session_row.id = v_session_id
      ...
      and not exists (
        ...
        where context_row.conversation_session_id = session_row.id
          and context_row.organization_id = session_row.organization_id
  ...
  if not v_session_ready then
    raise exception using
      errcode = 'P0001',
      message = 'Etapa 4.14 nao conseguiu criar sessao ativa sem contexto';
  end if;
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.14 ' || v_run_id::text,
        null,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_14',
  ...
  select exists (
    ...
    where message_row.id = v_message_id
      ...
      and not exists (
        select 1
        from public.commercial_session_context_links context_row
        where context_row.conversation_session_id = v_session_id
  ...
  insert into pg_temp._p9_414_result values (
    ...
    pg_catalog.format(
      'conversation_id=%s | session_id=%s | message_id=%s | session_ready=%s | insert_ok=%s | snapshot_ok=%s | sqlstate=%s | error=%s',
      v_conversation_id,
      v_session_id,
  ...
  if not v_cleanup_ok then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.14 cleanup failed; transaction rolled back (message_residue=%s, session_residue=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, identity_clean=%s)',
  ...
  insert into pg_temp._p9_414_result values (
    ...
    'PASS',
    pg_catalog.format(
      'runner_messages=%s | runner_sessions=%s | conversation_unchanged=%s | last_message_trigger_enabled=%s | fill_trigger_enabled=%s | identity_clean=%s',
      v_message_residue_count,
      v_session_residue_count,
      v_conversation_unchanged,
      v_last_message_trigger_enabled,
  ...
  when others then
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message = 'Etapa 4.14 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.14 safety gate falhou; transacao revertida';
    end if;
