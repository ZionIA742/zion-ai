-- ZION / Pilar 9 / Bloco 4 / Etapa 4.17
-- Um unico teste: preservar o snapshot da mensagem apos substituir o contexto.
--
-- Esperado:
-- - a mensagem nasce com commercial_context_capture_state = 'captured';
-- - conversation_session_id e o contexto original sao gravados;
-- - o contexto ativo e substituido pela funcao oficial;
-- - o contexto original passa para inactive e o substituto fica active;
-- - a mensagem anterior continua apontando para o contexto original.
--
-- O runner:
...
-- - confirma que o snapshot historico da mensagem anterior foi preservado;
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.17:replaced_context_snapshot_preservation',
    0
  )
);

create temp table _p9_417_result (
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_417_result values (
      1,
      'CHECK',
      'BLOCKED',
  ...
  begin
    ...
    select (
      public.link_commercial_session_context(
        ...
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_17',
  ...
  if not v_context_created or not v_context_ready then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.17 nao conseguiu criar contexto ativo (rpc_ok=%s, context_id=%s, ready=%s, sqlstate=%s, error=%s)',
        v_context_rpc_ok,
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.17 ' || v_run_id::text,
        null,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_17',
  ...
  select exists (
    ...
    where old_context.id = v_context_link_id
      ...
      and old_context.unlink_reason_code =
        'runner_context_replacement'
      and old_context.unlink_reason = 'runner Etapa 4.17'
  ...
  select exists (
    ...
    where message_row.id = v_message_id
      ...
      and exists (
        select 1
        from public.commercial_session_context_links new_context
        where new_context.id = v_replacement_context_link_id
          and new_context.replaces_link_id = v_context_link_id
          and new_context.status = 'active'
      )
  )
  ...
  insert into pg_temp._p9_417_result values (
    ...
    pg_catalog.format(
      'conversation_id=%s | opportunity_id=%s | session_id=%s | original_context_link_id=%s | replacement_context_link_id=%s | message_id=%s | context_ready=%s | insert_ok=%s | initial_snapshot_ok=%s | replace_ok=%s | replacement_chain_ok=%s | snapshot_preserved=%s | context_sqlstate=%s | context_error=%s | insert_sqlstate=%s | insert_error=%s | replace_sqlstate=%s | replace_error=%s',
      v_conversation_id,
      v_opportunity_id,
      v_session_id,
      coalesce(v_context_link_id::text, '<null>'),
  ...
  if not v_cleanup_ok then
    raise exception using
      errcode = 'P0001',
      message = pg_catalog.format(
        'Etapa 4.17 cleanup failed; transaction rolled back (messages=%s, contexts=%s, sessions=%s, opportunities=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, context_trigger=%s, identity_clean=%s)',
...
exception
  when others then
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message =
            'Etapa 4.17 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.17 safety gate falhou; transacao revertida';
    end if;
