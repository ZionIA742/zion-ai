-- ZION / Pilar 9 / Bloco 4 / Etapa 4.18
-- Um unico teste: nova mensagem usa o contexto substituto ativo.
--
-- Esperado:
-- - um contexto original e substituido pela funcao oficial;
-- - o contexto original passa para inactive e o substituto fica active;
-- - a mensagem criada depois da substituicao nasce como captured;
-- - conversation_session_id aponta para a sessao ativa;
-- - commercial_session_context_link_id aponta para o contexto substituto.
--
-- O runner:
-- - reutiliza uma conversa real apenas como parent imutavel;
-- - cria uma oportunidade, uma sessao e um contexto original proprios;
-- - substitui o contexto via public.replace_commercial_session_context_link();
-- - cria uma mensagem outgoing posterior via public.insert_message();
-- - confirma que a nova mensagem capturou o contexto substituto ativo;
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended(
    'zion:p9:block4:step4.18:replacement_context_new_message_capture',
    0
  )
);

create temp table _p9_418_result (
...
declare
  ...
  v_replacement_context_link_id uuid;
  v_message_id uuid;
  ...
  v_insert_error text;
  v_post_replace_snapshot_ok boolean := false;

  v_replace_ok boolean := false;
  v_replace_sqlstate text;
  v_replace_error text;
  v_replacement_chain_ok boolean := false;
...
begin
  if current_user <> 'postgres'
    ...
    insert into pg_temp._p9_418_result values (
      1,
      'CHECK',
      'BLOCKED',
  ...
  begin
    ...
    select (
      public.link_commercial_session_context(
        ...
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_18',
  ...
  if not v_context_created or not v_context_ready then
    raise exception using
      ...
      message = pg_catalog.format(
        'Etapa 4.18 nao conseguiu criar contexto ativo (rpc_ok=%s, context_id=%s, ready=%s, sqlstate=%s, error=%s)',
        v_context_rpc_ok,
  ...
  begin
    ...
    select (
      public.replace_commercial_session_context_link(
        ...
        'human',
        v_member_user_id,
        'runner Etapa 4.18 replacement',
        'runner:' || v_run_id::text || ':replacement',
        v_run_id,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_18',
          'runner_run_id', v_run_id::text,
          'runner_phase', 'replacement'
        ),
        'runner_context_replacement',
        'runner Etapa 4.18',
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_18',
          'runner_run_id', v_run_id::text,
          'runner_phase', 'original'
        ),
        null
      )
    ).id
    into v_replacement_context_link_id;

    v_replace_ok := true;
  exception
    when others then
      get stacked diagnostics
        v_replace_sqlstate = returned_sqlstate,
        v_replace_error = message_text;
      v_replace_ok := false;
  end;
  ...
  select exists (
    select 1
    from public.commercial_session_context_links old_context
    join public.commercial_session_context_links new_context
      on new_context.replaces_link_id = old_context.id
     and new_context.organization_id = old_context.organization_id
     and new_context.store_id = old_context.store_id
     and new_context.conversation_session_id =
       old_context.conversation_session_id
    where old_context.id = v_context_link_id
      and new_context.id = v_replacement_context_link_id
      and old_context.organization_id = v_organization_id
      and old_context.store_id = v_store_id
      and old_context.conversation_session_id = v_session_id
      and old_context.customer_id = v_customer_id
      and old_context.commercial_opportunity_id = v_opportunity_id
      and old_context.lead_customer_link_id = v_lead_customer_link_id
      and old_context.status = 'inactive'
      and old_context.unlinked_at is not null
      and old_context.unlinked_by_actor_type = 'human'
      and old_context.unlinked_by_user_id = v_member_user_id
      and old_context.unlink_reason_code =
        'runner_context_replacement'
      and old_context.unlink_reason = 'runner Etapa 4.18'
      and old_context.metadata #>> '{unlink,correlation_id}' =
        v_run_id::text
      and new_context.customer_id = v_customer_id
      and new_context.commercial_opportunity_id = v_opportunity_id
      and new_context.lead_customer_link_id = v_lead_customer_link_id
      and new_context.status = 'active'
      and new_context.linked_by_actor_type = 'human'
      and new_context.linked_by_user_id = v_member_user_id
      and new_context.correlation_id = v_run_id
      and new_context.metadata #>> '{replacement,replaces_link_id}' =
        v_context_link_id::text
      and new_context.metadata #>> '{replacement,reason_code}' =
        'runner_context_replacement'
      and new_context.linked_at >= old_context.unlinked_at
      and (
        select count(*)
        from public.commercial_session_context_links active_context
        where active_context.organization_id = v_organization_id
          and active_context.store_id = v_store_id
          and active_context.conversation_session_id = v_session_id
          and active_context.status = 'active'
      ) = 1
  )
  into v_replacement_chain_ok;
  ...
  begin
    ...
    select (
      public.insert_message(
        ...
        'outgoing',
        'text',
        'runner Etapa 4.18 after replacement ' || v_run_id::text,
        null,
        null,
        pg_catalog.jsonb_build_object(
          'runner_name', 'p9_block4_step4_18',
          ...
          'runner_context_link_id', v_replacement_context_link_id::text
        )
      )
    ).id
    into v_message_id;
  ...
  select exists (
    ...
    where message_row.id = v_message_id
      ...
      and exists (
        select 1
        from public.commercial_session_context_links context_row
        where context_row.id = v_replacement_context_link_id
          and context_row.organization_id = v_organization_id
          and context_row.store_id = v_store_id
          and context_row.conversation_session_id = v_session_id
          and context_row.status = 'active'
  ...
  into v_post_replace_snapshot_ok;
  ...
  insert into pg_temp._p9_418_result values (
    ...
    case
      when v_opportunity_created
       ...
       and v_post_replace_snapshot_ok
       and v_replace_ok
       and v_replacement_context_link_id is not null
       and v_replacement_chain_ok
      then 'PASS'
      else 'SUT_FAIL'
    end,
    pg_catalog.format(
      'conversation_id=%s | opportunity_id=%s | session_id=%s | original_context_link_id=%s | replacement_context_link_id=%s | message_id=%s | context_ready=%s | insert_ok=%s | post_replace_snapshot_ok=%s | replace_ok=%s | replacement_chain_ok=%s | context_sqlstate=%s | context_error=%s | insert_sqlstate=%s | insert_error=%s | replace_sqlstate=%s | replace_error=%s',
      v_conversation_id,
      v_opportunity_id,
      v_session_id,
      coalesce(v_context_link_id::text, '<null>'),
      coalesce(v_replacement_context_link_id::text, '<null>'),
      coalesce(v_message_id::text, '<null>'),
      v_context_ready,
      v_insert_ok,
      v_post_replace_snapshot_ok,
      v_replace_ok,
      v_replacement_chain_ok,
      coalesce(v_context_sqlstate, '<null>'),
      coalesce(v_context_error, '<null>'),
      coalesce(v_insert_sqlstate, '<null>'),
      coalesce(v_insert_error, '<null>'),
      coalesce(v_replace_sqlstate, '<null>'),
      coalesce(v_replace_error, '<null>')
    ),
    null
  );
  ...
  where context_row.id = v_replacement_context_link_id
    and context_row.correlation_id = v_run_id
    and context_row.replaces_link_id = v_context_link_id;
  ...
  if not v_cleanup_ok then
    raise exception using
      errcode = 'P0001',
      message = pg_catalog.format(
        'Etapa 4.18 cleanup failed; transaction rolled back (messages=%s, contexts=%s, sessions=%s, opportunities=%s, conversation_unchanged=%s, last_message_trigger=%s, fill_trigger=%s, context_trigger=%s, identity_clean=%s)',
...
exception
  when others then
    ...
    begin
      ...
      delete from public.commercial_session_context_links context_row
      where context_row.replaces_link_id = v_context_link_id
        and context_row.correlation_id = v_run_id;
    ...
    exception
      when others then
        raise exception using
          errcode = 'P0001',
          message =
            'Etapa 4.18 cleanup defensivo falhou; transacao revertida';
    end;
    ...
    if v_message_residue_count <> 0
      ...
      raise exception using
        errcode = 'P0001',
        message = 'Etapa 4.18 safety gate falhou; transacao revertida';
    end if;
